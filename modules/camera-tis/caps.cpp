...
std::vector<image_size> get_standard_resolutions(const image_size &min, const image_size &max, const image_size &step)
{
...
std::vector<double> index_framerates(
    GstElement &element,
...
std::vector<double> Caps::get_framerates(
...
std::vector<struct caps_format> Caps::generate_from_caps_list(GstElement &element)
{
    ...
    for (unsigned int i = 0; i < gst_caps_get_size(p_caps); ++i) {
        ...
        if (!std::any_of(resolutions.begin(), resolutions.end(), [&min](const image_size &res) {
                return res == min;
            })) {
            resolutions.insert(resolutions.begin(), min);
        }

        if (!std::any_of(resolutions.begin(), resolutions.end(), [&max](const image_size &res) {
                return res == max;
            })) {
