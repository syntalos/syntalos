...
void PropertiesBox::initialize_dialog(TcamCollection &collection)
{
    ...
    for (const std::string &name : names) {
        ...
        case TCAM_PROPERTY_TYPE_FLOAT: {
            ...
            connect(
                ptr,
                &DoubleWidget::update_category,
                p_worker,
                &PropertyWorker::update_category,
                Qt::QueuedConnection);
    ...
    connect(ui->tabWidget, &QTabWidget::currentChanged, this, &PropertiesBox::update_tab);
    // updates shall be done in another context
    connect(this, &PropertiesBox::update_category, p_worker, &PropertyWorker::update_category);
}
