//----------------------------------------------------------------------------------
// Rhd2000ONIDataBlock.cpp
//
// Intan Technoloies RHD2000 Rhythm Interface API
// Rhd2000ONIDataBlock Class
...
Rhd2000ONIDataBlock::Rhd2000ONIDataBlock (int numDataStreams, bool usb3) : samplesPerBlock (SAMPLES_PER_DATA_BLOCK (usb3)),
                                                                           usb3 (usb3)
{
...
void Rhd2000ONIDataBlock::allocateIntArray1D (std::vector<int>& array1D, int xSize)
{
    array1D.resize (xSize);
}

// Allocates memory for a 1-D array of unsigned integers.
void Rhd2000ONIDataBlock::allocateUIntArray1D (std::vector<unsigned int>& array1D, int xSize)
{
    array1D.resize (xSize);
}

// Allocates memory for a 2-D array of integers.
void Rhd2000ONIDataBlock::allocateIntArray2D (std::vector<std::vector<int>>& array2D, int xSize, int ySize)
{
    int i;
...
void Rhd2000ONIDataBlock::allocateIntArray3D (std::vector<std::vector<std::vector<int>>>& array3D, int xSize, int ySize, int zSize)
{
    int i, j;
...
unsigned int Rhd2000ONIDataBlock::getSamplesPerDataBlock (bool usb3)
{
...
unsigned int Rhd2000ONIDataBlock::calculateDataBlockSizeInWords (int numDataStreams, bool usb3, int nSamples)
{
...
bool Rhd2000ONIDataBlock::checkUsbHeader (unsigned char usbBuffer[], int index)
{
...
unsigned int Rhd2000ONIDataBlock::convertUsbTimeStamp (unsigned char usbBuffer[], int index)
{
...
int Rhd2000ONIDataBlock::convertUsbWord (unsigned char usbBuffer[], int index)
{
...
void Rhd2000ONIDataBlock::fillFromUsbBuffer (unsigned char usbBuffer[], int blockIndex, int numDataStreams, int nSamples)
{
...
// to the console.
void Rhd2000ONIDataBlock::print (int stream) const
{
    ...
    std::cout << "    Chip Name: " << (char) auxiliaryData[stream][2][24] << (char) auxiliaryData[stream][2][25] << (char) auxiliaryData[stream][2][26] << (char) auxiliaryData[stream][2][27] << (char) auxiliaryData[stream][2][28] << (char) auxiliaryData[stream][2][29] << (char) auxiliaryData[stream][2][30] << (char) auxiliaryData[stream][2][31] << std::endl;
    std::cout << "    Company Name:" << (char) auxiliaryData[stream][2][32] << (char) auxiliaryData[stream][2][33] << (char) auxiliaryData[stream][2][34] << (char) auxiliaryData[stream][2][35] << (char) auxiliaryData[stream][2][36] << std::endl;
    ...
    std::cout << "    tempS2, tempS1:        " << ((auxiliaryData[stream][2][RamOffset + 3] & 0x10) >> 4) << "," << ((auxiliaryData[stream][2][RamOffset + 3] & 0x08) >> 3) << std::endl;
    ...
    std::cout << "  Temperature sensor (only one reading): " << tempUnitsC << " C (" << tempUnitsF << " F)" << std::endl;
...
void Rhd2000ONIDataBlock::writeWordLittleEndian (std::ofstream& outputStream, int dataWord) const
{
