...
# Unset WAYLAND_DISPLAY so that Qt/GTK applications use the Xvfb X11 server
# instead of trying to connect to the Wayland compositor (which would fail or
# bypass Xvfb entirely when running on a Wayland desktop or in CI).
set +e
DISPLAY=:$SERVERNUM XAUTHORITY=$AUTHFILE WAYLAND_DISPLAY= "$@" 3>&-
RETVAL=$?
set -e
