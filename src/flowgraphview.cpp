...
void FlowGraphNodePort::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *)
{
...
{
    return static_cast<FlowGraphNodePort *>(m_portkeys.value(FlowGraphNodePort::ItemKey(name, mode, type), nullptr));
}
...
void FlowGraphNode::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *)
{
    const QPalette &pal = option->palette;
      ...
      m_edited(0)
{
    ...
    QObject::connect(m_editor, SIGNAL(textChanged(const QString &)), SLOT(textChanged(const QString &)));
...
bool FlowGraphView::canRenameItem(void) const
{
    ...
    return (item && (item->type() == FlowGraphNode::Type || item->type() == FlowGraphNodePort::Type));
}
 ...
 */
FlowGraphNode *FlowGraphView::findNode(const QString &name, FlowGraphItem::Mode mode, uint type) const
{
    return static_cast<FlowGraphNode *>(m_nodekeys.value(FlowGraphNode::ItemKey(name, mode, type), nullptr));
}
...
static QVariantList qrectfToVarList(const QRectF &rect)
{
    QVariantList rRect = {rect.x(), rect.y(), rect.width(), rect.height()};
    return rRect;
}

static QRectF qrectfFromVarList(const QVariant &val)
{
    const auto arr = val.toList();
    if (arr.size() != 4)
        return QRectF();
    QRectF rect(arr[0].toDouble(), arr[1].toDouble(), arr[2].toDouble(), arr[3].toDouble());
    return rect;
}

static QVariantList qpointfToVarList(const QPointF &point)
{
    QVariantList pl = {point.x(), point.y()};
    return pl;
}

static QPointF qpointfFromVarList(const QVariant &val)
{
    const auto arr = val.toList();
    if (arr.size() != 2)
        return QPointF();
    QPointF point(arr[0].toDouble(), arr[1].toDouble());
    return point;
}
...
bool FlowGraphView::restoreNodePos(FlowGraphNode *node)
{
    ...
    const auto vNodePos = m_settings.value(NodePosGroup).toHash();
    const QPointF &node_pos = qpointfFromVarList(vNodePos.value(nodeKey(node)));
...
bool FlowGraphView::saveNodePos(FlowGraphNode *node)
{
    ...
    auto vNodePos = m_settings.value(NodePosGroup).toHash();
    vNodePos.insert(nodeKey(node), qpointfToVarList(node->pos()));
    m_settings.insert(NodePosGroup, vNodePos);

    return true;
}

QVariantHash FlowGraphView::settings() const
{
    return m_settings;
}

void FlowGraphView::setSettings(const QVariantHash &settings)
{
    m_settings = settings;
}
...
bool FlowGraphView::restoreState()
{
    ...
    auto varCanvas = m_settings.value(CanvasGroup).toHash();
    const QRectF &rect = qrectfFromVarList(varCanvas.value(CanvasRectKey));
