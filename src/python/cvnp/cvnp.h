...
{
    return pybind11::capsule(new cv::Matx<_Tp, _rows, _cols>(m), [](void *v) {
        delete reinterpret_cast<cv::Matx<_Tp, _rows, _cols> *>(v);
    });
}
} // namespace detail
...
pybind11::array matx_to_nparray(const cv::Matx<_Tp, _rows, _cols> &m)
{
    return pybind11::array(pybind11::dtype::of<_Tp>(), std::vector<std::size_t>{_rows, _cols}, m.val);
}
    ...
    bool load(handle src, bool)
    {
        ...
        if (tupleSize > 4)
            throw std::invalid_argument(
                "Scalar should be a tuple with at most 4 elements. Got " + std::to_string(tupleSize));
