...
static void display_mat(const std::string &title, const cv::Mat &m)
{
    std::cout << title << "\n" << cv::format(m, cv::Formatter::FMT_PYTHON) << "\n";
}

static bool is_matrix_content_equal_to(const cv::Mat &m, const std::string &expectedFormattedContent)
{
    std::stringstream ss;
...
static void test_non_continuous_mat()
{
...
static void test_nparray_to_mat()
{
...
static void test_lifetime()
{
    ...
    auto create_example_array = []() -> pybind11::array {
        ...
        printf("Created array data address =%p\n%s\n", a.data(), py::str(a).cast<std::string>().c_str());
        return a;
    };
