...
static cv::Mat cvnp_roundtrip(const cv::Mat &m)
{
    return m;
}
...
struct CvNp_TestHelper {
    ...
    void SetM(int row, int col, uchar v)
    {
        m.at<uchar>(row, col) = v;
    }
    ...
    void set_m_double(int row, int col, double v)
    {
        m_double(row, col) = v;
    }
    ...
    void SetMX_ns(int row, int col, double v)
    {
        mx_ns(row, col) = v;
    }
    ...
    void SetM10(int row, int col, cv::Vec3f v)
    {
        m10.at<cv::Vec3f>(row, col) = v;
    }
    cv::Vec3f GetM10(int row, int col)
    {
        return m10.at<cv::Vec3f>(row, col);
    }
    cv::Mat GetSubM10()
    {
...
static cv::Matx33d ShortLivedMatx()
{
...
static cv::Matx33d make_eye()
{
    return cv::Matx33d::eye();
}

static void display_eye(cv::Matx33d m = cv::Matx33d::eye())
{
    printf("display_eye\n");
...
static void pydef_cvnp_test(pybind11::module &m)
{
