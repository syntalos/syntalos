...
#ifdef DEBUG_ALLOCATOR
            printf(
                "CvnpAllocator::deallocate() - not doing anything since urefcount=%d nbAllocations=%d\n",
                u->urefcount,
                nbAllocations);
#endif
        }
...
static py::dtype determine_np_dtype(int cv_depth)
{
    for (auto format_synonym : cvnp::sTypeSynonyms)
        if (format_synonym.cv_depth == cv_depth)
            return format_synonym.dtype();

    std::string msg = "numpy does not support this OpenCV depth: " + std::to_string(cv_depth)
                      + " (in determine_np_dtype)";
    throw std::invalid_argument(msg.c_str());
}

static int determine_cv_depth(pybind11::dtype dt)
{
    for (auto format_synonym : cvnp::sTypeSynonyms)
        if (format_synonym.np_format[0] == dt.char_())
            return format_synonym.cv_depth;

    std::string msg = std::string("OpenCV does not support this numpy format: ") + dt.char_()
                      + " (in determine_np_dtype)";
    throw std::invalid_argument(msg.c_str());
}

static int determine_cv_type(const py::array &a, int depth)
{
    if (a.ndim() < 2)
...
static cv::Size determine_cv_size(const py::array &a)
{
    if (a.ndim() < 2)
...
static std::vector<std::size_t> determine_strides(const cv::Mat &m)
{
    if (m.channels() == 1) {
        return {
...
static py::capsule make_capsule_mat(const cv::Mat &m)
{
    return py::capsule(new cv::Mat(m), [](void *v) {
...
pybind11::array mat_to_nparray(const cv::Mat &m)
{
    return pybind11::array(
        detail::determine_np_dtype(m.depth()),
...
static bool is_array_contiguous(const pybind11::array &a)
{
...
cv::Mat nparray_to_mat(pybind11::array &a)
{
    ...
    if (!is_contiguous && is_not_empty) {
        throw std::invalid_argument(
            "cvnp::nparray_to_mat / Only contiguous numpy arrays are supported. / Please use np.ascontiguousarray() to "
            "convert your matrix");
    }
