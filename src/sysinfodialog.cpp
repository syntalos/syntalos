      ...
      ui(new Ui::SysInfoDialog)
{
    ...
    if (sysInfo->syntalosHWSupportInstalled()) {
        ...
        ui->valSyHWSupport->setText(
            "<html>missing, can be <a href=\"https://syntalos.org/docs/setup/hardware-support/\">downloaded</a>.");
...
{
    auto infoText = QStringLiteral("Syntalos %1 | %2 (Kernel: %3) | QPA: %4\n")
                        .arg(
                            ui->valSyntalos->text(),
                            ui->valOSName->text(),
                            ui->valKernel->text(),
                            ui->valGuiPlatform->text());
