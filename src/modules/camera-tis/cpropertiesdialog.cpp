...
CPropertiesDialog::CPropertiesDialog(QWidget *parent)
    : QDialog(parent),
      m_cameraSet(false)
{
    ...
    connect(btnOk, &QPushButton::clicked, this, &CPropertiesDialog::onOK);
...
QVBoxLayout * CPropertiesDialog::findLayoutToAdd(const std::string Category)
{
    ...
    {
        for( size_t c = 0; c < _Pages[p]._Categories.size(); c++ )
        {
...
{
    for (size_t i = 0; i < _PropertySliders.size(); i++ ) {
...
{
    uint iValue = g_value_get_int(value);
...
void CPropertyCtrl_int::exposure_scale( int i_x_min,  int i_x_max, const int num_samples, const double)
{
...
void CPropertyCtrl_double::set_value(GValue *value)
{
    ...
    if( dscale.size() > 0  )
    {
        for( size_t i = 0; i < dscale.size() -1; i++ )
        {
...
void CPropertyCtrl_boolean::OnClickedCheckkBox(int)
{
...
void CPropertyCtrl_button::set_value(GValue *)
{
    // Nothing to do.
}
...
void CPropertyCtrl_enum::set_value(GValue *)
{
    // TODO
}
