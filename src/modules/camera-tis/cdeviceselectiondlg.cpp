...
*/
CDeviceSelectionDlg::CDeviceSelectionDlg(QWidget* parent )
    : QDialog(parent)
{
...
void CDeviceSelectionDlg::EnumerateDevices()
{
    ...
    const auto tcam = gst_element_factory_make("tcamsrc", "tcam");
    if (tcam == nullptr) {
        QMessageBox::warning(this,"TCam Capture","Can not instantiate tcamsrc.");
    }

    GSList *pSerials = tcam_prop_get_device_serials(TCAM_PROP(tcam));
    ...
    while( pSerials )
    {
        ...
        tcam_prop_get_device_info(TCAM_PROP(tcam) , Dev->SerialNumber.c_str() , &Name, &Identifier, &Connection);
        Dev->Name = Name;
...
void CDeviceSelectionDlg::OncboCamerasChanced(int Index)
{
    ...
    for( size_t i = 0; i < _VideoFormats.size(); i++ )
    {
...
bool CDeviceSelectionDlg::EnumerateVideoFormats(std::string SerialNumber )
{
    ...
    if( pad_caps != NULL )
    {
        for (size_t i = 0; i < gst_caps_get_size (pad_caps); i++)
        {
...
void CDeviceSelectionDlg::OncboVideoFormatsChanced(int Index)
{
    ...
    for(size_t i = 0; i < pVF->_framerates.size();i++ )
    {
...
CVideoFormat::CVideoFormat(GstStructure *pStructure)
{
    ...
    else
    {
        if( strlen(buffer) != 0 && strlen(buffer) < 24)
        {
            ...
            {
                for( size_t i = 0; i < gst_value_list_get_size(value); i++ )
                {
